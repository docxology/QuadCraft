# 4D Go

## Overview
Territory-capturing strategy game on 4D IVM grid with 12 kissing-sphere directions. Part of [QuadCraft](../).

## Quick Start
To play this game, run the following from the project root:

```bash
python3 games/run_games.py --game go
```

## Documentation
- **Agent Instructions**: [AGENTS.md](AGENTS.md) - Technical details and architecture.
- **Game Index**: [Index](../GAMES_INDEX.md) - List of all QuadCraft games.

## Verify
Run the test suite for this game:
```bash
python3 games/run_games.py --test --game go
```
