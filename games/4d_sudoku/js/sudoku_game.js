...
class SudokuGame extends BaseGame {
    ...
    _handleClick(e) { const rect=this.canvas.getBoundingClientRect(); const cell=this.renderer.hitTest(e.clientX-rect.left,e.clientY-rect.top,this.camera); if(cell){this._lastClick=cell;this.board.place(cell,this.selectedVal);this._render();} }
    _handleKey(e) { const n=parseInt(e.key); if(n>=1&&n<=9){this.selectedVal=n;this._render();} if(e.key==='0'){if(this._lastClick){this.board.place(this._lastClick,0);this._render();}} if(e.key==='r'){this.board.reset();this._render();} }
