"""
validation.py — Structural and logical validation for QuadCraft game directories.
...
SHARED_MODULES = ["quadray.js", "camera.js", "projection.js", "zoom.js"]
...
def validate_game(games_dir: Path, key: str) -> list[str]:
    ...
    if key not in ES_MODULE_GAMES:
        ...
        # 4. index.html imports shared modules
        if index.exists():
...
def audit_all(games_dir: Path) -> bool:
    ...
    if shared_issues:
        ...
        print(f"  ✅ {SHARED_DIR_NAME}/ — all {len(SHARED_MODULES)} shared modules present")
